...
static Bin_tree_node *ADD_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == ADD_OPERATION);

    return ADD_(DIFF(src->left), DIFF(src->right));
}

static Bin_tree_node *SUB_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == SUB_OPERATION);

    return SUB_(DIFF(src->left), DIFF(src->right));
}

static Bin_tree_node *MLT_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == MLT_OPERATION);

    return ADD_(MLT_(DIFF(src->left), COPY(src->right)),
                MLT_(COPY(src->left), DIFF(src->right)));
}

static Bin_tree_node *DIV_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == DIV_OPERATION);
...
static Bin_tree_node *LN_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                       errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == LN_OPERATION);
    assert(!src->left);

    return DIV_(DIFF(src->right), COPY(src->right));
}

static Bin_tree_node *POW_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == POW_OPERATION);
    ...
    if (src->right->data.type == EXPRESSION_TREE_LITERAL_TYPE) {
...
static Bin_tree_node *SIN_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == SIN_OPERATION);
    assert(!src->left);

    return MLT_(COS_(COPY(src->right)), DIFF(src->right));
}

static Bin_tree_node *COS_differentiate(Bin_tree_node const *const src, char const *const main_var,
                                        errno_t *const err_ptr) {
    assert(src);
    assert(src->data.type == EXPRESSION_TREE_OPERATION_TYPE);
    assert(src->data.val.operation == COS_OPERATION);
    assert(!src->left);
...
Bin_tree_node *subtree_differentiate(Bin_tree_node const *const src, char const *const main_var,
    ...
    switch (src->data.type) {
        ...
        case EXPRESSION_TREE_OPERATION_TYPE:
            switch (src->data.val.operation) {
                ...
                case name ## _OPERATION:                                    \
                    return name ## _differentiate(src, main_var, err_ptr);
        ...
        case EXPRESSION_TREE_ID_TYPE:
            ...
            if (!strcmp(src->data.val.name, main_var)) {
                return LITER_(1);
            }
